...
6. **`Start`에는 반드시 `-compile`을 붙인다**(2026-09-10 사용자 실기 관측). 제어기의 `Start`는 스위치 없이는 컴파일하지 않고 **직전에 컴파일된 옛 바이너리를 그대로 실행한다** — `/GPL`에 새 소스를 올려도 반영되지 않는다. 캡처의 GDE도 `Load … → COMPILE <proj> → Start <proj> -event` 순으로 Start 앞에 명시적 `COMPILE`을 보냈다(옛 규칙은 이 `-event`만 보고 "자체 컴파일"로 오독한 것). 확장은 `startCommand.buildStartCommand`가 `-compile`을 기본으로 붙인다. 여전히 **Compile 직후 Start를 연속으로 보내지 않는다(컴파일 중복, 한 번에 하나만)**: 에러 확인은 `Compile`(또는 `GPL: Deploy`), 실행은 `Start`(`GPL: Start`)만. Brooks 공식 문서는 가설로 다루고 실기기 응답을 사실로 삼는다(`docs/ai-handoff.md` §0.7). 확장 경로(명령·`gpl.ai.debug.*`·URI)로 보낼 때는 명령 정책(`src/controller/commandPolicy.ts`)이 이 조건과 Step 연타·정지 정착(§0.6)을 대신 기다려 충족시키므로 호출자가 따로 지킬 필요가 없다 — raw TCP/MCP 직접 전송 때만 스스로 지킨다.
...
2. 최신 코드 검증
   - 먼저 `GPL: Deploy (/GPL 업로드 + Compile, Start 없음)` (`gpl.deploy`)로 UPLOAD ∥ STOP -> COMPILE 단계만 확인한다(업로드와 정지는 동시 진행, 둘 다 끝난 뒤 Compile).
...
| 붙기만 | GPL: Debug: Attach Only | `gpl.debug.attachOnly` | 이미 실행 중인 프로그램에 붙기(제어기 상태 안 바꿈) |
| 배포 + 붙기 | GPL: Debug Project | `gpl.debugProject` | 대상을 QuickPick·탐색기 우클릭으로 고른 뒤 배포하고 붙기 |
