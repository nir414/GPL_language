...
- **「업로드 스타트」 버튼**(GPL Controller 패널, 로켓 아이콘) — 정지 → `/GPL` 업로드 → 실행을 한 번에.
  `Compile`은 보내지 않습니다(컴파일 중복을 피하기 위함). ※ 2026-09-10 정정: 당시 `Start`에는 `-compile`도
  붙지 않아 실제로는 컴파일이 아예 이뤄지지 않고 옛 바이너리가 실행됐습니다 — 이후 버전에서 `Start`에
  `-compile`을 기본으로 붙여 해결했습니다.
