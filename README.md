...
- **실시간 로그 터미널** — 1402/1403 트래픽을 VS Code 터미널에 미러링 (⚠ 1403 수신은 정상 동작 미확보 — 아래 "실시간 로그 터미널" 절 참고)
...
> ⚠️ **알려진 제한 — 1403 수신 비정상**: 1403 출력 이벤트 수신은 실기기에서 안정적인 동작을
> 확보하지 못했습니다(연결은 되지만 payload가 수신되지 않는 경우가 많음). 원인을 확정하지 못해
> 개선 작업은 사실상 중단된 상태이며, `[1403]` 출력은 있는 그대로(as-is) 제공됩니다.
> `[1402]` 명령 트래픽 미러링은 사용할 수 있습니다.
