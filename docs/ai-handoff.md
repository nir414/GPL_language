...
- [ ] (기록만, 미적용) `deployService.ts` -745/-508 복구 분기에서 재시도 실패(`cr2`)의 `lastCompileFailure`가 공통 폴스루의 원래 `cr` STATUS로 덮어써짐 — 실패 보고 정확성 이슈(동작 무해).
- [ ] (문서 갱신) §3-B B1의 라인 참조: `Set Nobreak ... "file" line`(공백 O)는 현재 `gplDebugSession.ts:461` 부근(여전히 미해결, no-space 3곳과 불일치).
