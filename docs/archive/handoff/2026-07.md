...
- 수록 범위: **§1-C ~ §1-AL** (2026-07-03 ~ 2026-07-31, 35개 세션)
- 전체 세션 인덱스: `docs/ai-handoff.md` §1

## 목차

- **§1-C** (2026-07-03) — VSIX 패키징 실패(EACCES) 해결 + 패키징 파이프라인 개선
- **§1-D** (2026-07-03) — 디버그 스텝 체감 지연 개선
- **§1-E** (2026-07-03) — 디버그 변수 확인 UX: 클릭 즉시 표시
- **§1-F** (2026-07-03) — 제어기 무응답 사건 + LSP 정리
- **§1-G** (2026-07-08) — Quick Compile 재설계: /GPL 직접 업로드 + Stop 완료 게이트
- **§1-H** (2026-07-08) — 디버그 `<projectName>` 오인식(다른 프로젝트로 처리) 수정
- **§1-I** (2026-07-08) — 디버그(F5) 배포: /GPL 직접 미러 동기화 (flash 미경유)
- **§1-J** (2026-07-10) — 언어 서비스 개선(Hover/IntelliSense/Signature Help) + Brooks 사전 대폭 확장
- **§1-K** (2026-07-13) — 정의찾기(F12) 오버로드 해석: 인자 타입 추론 + 동점 peek
- **§1-L** (2026-07-13) — 디버그 브레이크 감지/전환 체감 개선 + F8 키바인딩 충돌 수정
- **§1-M** (2026-07-14) — 호버 팝업 스팸 개선 (요약 모드 + 디버그 중 간소화 + gpl.hover.* 설정)
- **§1-N** (2026-07-14) — 디버그(F5) 배포에 "업로드 전 쓰레드 확인 + 정지 확인 모달" 게이트 적용
- **§1-O** (2026-07-14) — 반복되는 `.git/index.lock` "File exists" 에러 진단 + 해제 스크립트 추가
- **§1-P** (2026-07-14) — 디버그 변수 표시: 배열/객체 구조적(트리) 표시 지원
- **§1-Q** (2026-07-16) — 자체 검토: 전체 코드 리뷰 + §3-B 보류 항목 일괄 적용
- **§1-R** (2026-07-16) — 자동완성 개선: 멤버 완성 + 로컬 완성 + 중복 삽입 방지
- **§1-S** (2026-07-16) — 중첩 클래스 파서 수정 + 스모크 하니스 + Dictionary 커버리지 대조
- **§1-T** (2026-07-22) — 정의 찾기: `Shared Public Dim` 수식어 순서 + 문자열 속 프로시저 참조(New Thread)
- **§1-U** (2026-07-22) — Show Variable 실기기 검증(§1-P 후속): 객체 헤더 형식 차이 수정 + 콘솔 평가 한계 확인
- **§1-V** (2026-07-22) — 디버깅 중 엉뚱한 폴더 파일이 열리는 문제 수정 (소스맵 경합 해소)
- **§1-W** (2026-07-22) — 디버그 hover에서 `armList(i)` 같은 인덱스 식 평가 지원
- **§1-X** (2026-07-22) — Globals 패널 표시 지연 진단·개선
- **§1-Y** (2026-07-22) — 실기기 추가 검증 반영: 객체 배열 분류 + 점 표기 멤버 폴백
- **§1-Z** (2026-07-23) — 0.8.0 릴리즈 메타데이터 정리 + 검증/패키징
- **§1-AA** (2026-07-23) — 버전/커밋/태그/릴리즈 운영 문서 정리
- **§1-AB** (2026-07-23) — 1403 수신 비정상 상태 문서 명시 + 릴리즈 문서 표현 정리
- **§1-AC** (2026-07-24) — AI Debug Assist 오케스트레이션 명령 추가
- **§1-AD** (2026-07-24) — AI 자율 디버깅 API/루프 추가
- **§1-AF** (2026-07-24) — 배포 경로 이원화: Deploy=/GPL 직접, Save to Flash 신설, Start 버튼 분리
- **§1-AG** (2026-07-24) — AI 자율 디버깅 API 견고화 (반환 계약/Output 기록/pause 폴링)
- **§1-AH** (2026-07-24) — 외부 AI(Claude Code) 실전 투입 관찰: MCP 미등록 → 원시 TCP 우회 (분석만, 코드 변경 없음)
- **§1-AI** (2026-07-28) — src/ 전체 가독성 정리 (동작 불변 리팩터링만)
- **§1-AJ** (2026-07-28) — GPL Controller 뷰 타이틀 툴바 재구성 (package.json만)
- **§1-AK** (2026-07-28) — 확장 트리 ↔ 디버그 패널 쓰레드 기능 병합
- **§1-AL** (2026-07-31) — 트리 "현재 실행 위치 보기"가 .history stale 사본을 열던 버그 수정
